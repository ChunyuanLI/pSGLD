...
dataFactory = MnistData(Ntrain,Ntest,opts);
data = dataFactory();
...
linearSizes = [500 data.outSize]; % first linear inSize will be set to the outputsize of the last convolutional layer.
...
ex = ConvolutionExperiment('AdaSpectral',NNFactory,dataFactory,@AdaSpectral);
ex.descentOpts.gpu = gpu;
ex.descentOpts.learningRate = 1e-5;
exps{numel(exps)+1} = ex;
...
ex = ConvolutionExperiment('AdaGrad',NNFactory,dataFactory,@Adagrad);
ex.descentOpts.gpu = gpu;
ex.descentOpts.learningRate = .0003;
exps{numel(exps)+1} = ex;

ex = ConvolutionExperiment('Rmsprop',NNFactory,dataFactory,@RMSprop);
ex.descentOpts.gpu = gpu;
ex.descentOpts.learningRate = .0001;
exps{numel(exps)+1} = ex;
