function [err,state] = AdaSpectral2(model,f,opts,state) %--TODO: Add momentum
    ...
    for l=1:L
        if isa((model.modules{l}),'Linear')
            ...
            if isa((model.modules{l}.weights),'gpuArray')
                state.history{l}=gpuArray.zeros(siz);
                state.biasHistory{l}=gpuArray.zeros(siz(1),1);
            else
                state.history{l}=zeros(siz);
                state.biasHistory{l}=zeros(siz(1),1);
            end
...
%             if opts.reportL2Penalty; err = err + 0.5*opts.weightDecay*dot(model.modules{l}.weights(:),model.modules{l}.weights(:)); end
 ...
 state.history{l} = state.history{l}+ *matr.^2;
        state.biasHistory{l} = state.biasHistory{l} + bias.^2;
