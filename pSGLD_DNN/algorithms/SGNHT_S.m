function [err,state] = SGNHT_S(model,f,opts,state) %--TODO: Add momentum
%% error checking and initialization
    ...
    state.thermostat = state.thermostat + (state.momentum.*state.momentum-1) * lr/2;
    state.momentum   = exp(-state.thermostat* lr/2) .* state.momentum - opts.N * grad * lr/2;
