function [err,state] =RMSSpectral2(model,f,opts,state) %--TODO: Add momentum
...
for l=1:L
    ...
    elseif isa(model.modules{l},'SpatialConvolution') && (isfield(opts,'projectKernels') && opts.projectKernels)
        ...
        state.history{l} = rmsd*state.history{l}+ H.^2;
        history=eps+sqrt(state.history{l});
        hist2=sqrt(history);
        k=min(min(size(history)),30);
        H = lr*projectKernels(H./hist2,opts.gpu,'approxRandSpectral',k)./hist2;
