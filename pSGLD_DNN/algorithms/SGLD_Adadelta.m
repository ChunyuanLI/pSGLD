function [err,state] = SGLD_Adadelta(model,f,opts,state) %--TODO: Add momentum
%% error checking and initialization
    ...
    % G =1./ (eps + sqrt(state.history));

    delta_params = lr* grad./G;
    param_change = delta_params + sqrt(2*lr./G).*randn(size(grad));
    params = params - param_change;
    
    state.history_delta = rmsd*state.history_delta + (1-rmsd)*param_change.^2;
