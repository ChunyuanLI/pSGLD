function [err,state] = SGLD_ADAMmax(model,f,opts,state) %--TODO: Add momentum
%% error checking and initialization
    ...
    state.history2 = max(rmsd2*state.history2, abs(grad));
    moment1 = state.history1/(1-rmsd1^state.iter);
    moment_max = state.history2;
    
    pcder= 1./ (eps + moment_max);
